...
import { getQaForUnassign } from "../api/userApi";
 import { unAssignQa } from "../api/userApi";
const UnAssignedQa=()=>{
...
console.log(item, "atom");
  ...
  const handleCheckboxChange = (option) => {
    ...
    const getQaForunassign= async()=>{
      try {
        const res = await getQaForUnassign({ project_id: item.project_id });
        setUsers(res.res);
      } catch (error) {
        console.log(error);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await unAssignQa({ project_id: item.project_id, qa_id: selectedOptions});
      navigate("/home")
  ...
  return (
    <div className="container" style={{ width: 350, marginTop: 50 }}>
      ...
      <form onSubmit={handleSubmit} >
        ...
        <div>
          <div onClick={handleSelectorClick} style={{ cursor: "pointer", fontSize:20 }}>Click here to select Qa</div>
          {showCheckboxes && (
            <div>
              { users.length > 0 ?  users.map((item) => (
                <div key={item.user_id}>
                  ...
                  /> </div>
              )) : <h4 style={{color:"red" , marginTop:15}}>No QA Found</h4>
            }
            </div>
          )}
        </div>
